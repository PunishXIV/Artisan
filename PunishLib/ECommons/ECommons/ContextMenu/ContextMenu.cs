...
using ECommons.ContextMenu.Inventory;
...
namespace ECommons.ContextMenu
{
    ...
    public class ContextMenu : IDisposable
    {
        ...
        private unsafe AtkValue* ExpandContextMenuArray(IntPtr addon)
        {
            ...
            var newArray = uiAlloc.Alloc(size);
            // zero new memory
            ...
            *(ushort*)(addon + AddonArraySizeOffset) = (ushort)newItemCount;
            ...
            if (oldArray != null)
            {
                ...
                uiAlloc.Free((IntPtr)oldArray - 8);
            }
        ...
        private void FreeSubMenuTitle()
        {
            ...
            uiAlloc.Free(this.SubMenuTitle);
        ...
        private unsafe bool SubMenuInner(IntPtr agent)
        {
            ...
            *selectedIdx = 0xFF;
            ...
            this.SubMenuTitle = uiAlloc.Alloc((ulong)nameBytes.Length);
    ...
    internal static class SigScannerExtensions
    {
        internal static bool TryScanText(this SigScanner s, string sig, out IntPtr ptr, string str)
        {
            return s.TryScanText(sig, out ptr);
        }
    }
}